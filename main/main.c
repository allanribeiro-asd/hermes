...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param){
	...
	switch (event)
	{
		case ESP_SPP_INIT_EVT:
			...
			ESP_LOGD(SPP_TAG, "Call esp_bt_dev_set_device_name(EXAMPLE_DEVICE_NAME)");
			esp_bt_dev_set_device_name(EXAMPLE_DEVICE_NAME);
			ESP_LOGD(SPP_TAG, "Call esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_GENERAL_DISCOVERABLE)");
			esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_GENERAL_DISCOVERABLE);
			ESP_LOGD(SPP_TAG, "Call esp_spp_start_srv(sec_mask,role_slave, 0, SPP_SERVER_NAME)");
