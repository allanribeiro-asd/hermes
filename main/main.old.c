...
#define SPP_SHOW_DATA 0 // Constant macro to assert if data must be shown. do not change this
#define SPP_SHOW_SPEED 1 // Cosntant macro to assert if speed must be shown. do not change this

#define SPP_SHOW_MODE SPP_SHOW_DATA // Choose either SPP_SHOW_DATA or SPP_SHOW_SPEED
 ...
 *
 *************************************************************************/
static char *bda2str(uint8_t * ptr, char *str, size_t size)
{
	if (ptr == NULL || str == NULL || size < 18)
	{
		return NULL;
	}
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
	...
	switch (event)
	{
		...
		case ESP_SPP_INIT_EVT:
			...
			{
				ESP_LOGI(SPP_TAG, "ESP_SPP_INIT_EVT ESP inicializado com sucesso");
		...
		case ESP_SPP_DATA_IND_EVT:
					 ...
					 */
					ESP_LOGI(
						SPP_TAG, "ESP_SPP_DATA_IND_EVT len:%d handle:%"PRIu32,
						param->data_ind.len,
...
;
					if (param->data_ind.len < 128)
					{
						...
						{
							printf("%c", (char)*(param->data_ind.data+i));
						}
		...
		case ESP_SPP_SRV_OPEN_EVT:
			ESP_LOGI(
				SPP_TAG,
				"ESP_SPP_SRV_OPEN_EVT status:%d handle:%"PRIu32", origin:[%s]",
...
void app_main(void)
{
	...
	ESP_LOGI(SPP_TAG, "Endereço: %s", bda2str((uint8_t *)esp_bt_dev_get_address(), bda_str, sizeof(bda_str)));
}
